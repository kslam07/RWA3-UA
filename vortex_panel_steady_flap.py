...
def aijmatrix(xi, yi, xj, yj, Npan, ni):  # Calculation of the influence coefficients
    ...
    for i in range(0, Npan):
        ...
        for j in range(0, Npan):
            vel_vor = lumpvor2d(xi[i], yi[i], xj[j], yj[j])
            a_ij = vel_vor @ ni[:, i]
            a_mat[i, j] = a_ij
...
@nb.njit
def compute_velocity_field(u_inf, X, Y, x_vort, y_vort, gamma_arr):

    # Build velocity and pressure distribution
    u = np.ones((len(X), len(Y))) * u_inf
    ...
    for i in range(len(X[:, 1])):
        ...
        for j in range(len(Y[1, :])):
            for g, gamma in enumerate(gamma_arr):
                uv = lumpvor2d(gamma, X[i, j], Y[i, j], x_vort[g], y_vort[g])
                u[i, j] = u[i, j] + uv[0]
                v[i, j] = v[i, j] + uv[1]
...
if plot_velocity_field:
    ...
    vmin = int(round(men - 0.5 * rms))
    vmax = int(round(men + 0.5 * rms))
